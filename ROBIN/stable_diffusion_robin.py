...
class ROBINStableDiffusionPipeline(StableDiffusionPipeline):
            ...
            for epoch in range(num_train_epochs):
                for step, batch in enumerate(dataloader):
                    with accelerator.accumulate(unet):
                        ...
                        image = (2.0 * gt_tensor - 1.0).float()

                        # Keep the latent/FFT path in full precision; fp16 FFT/VAE decode is a common NaN source.
                        with torch.autocast(device_type=accelerator.device.type, enabled=False):
                            latents = vae.encode(image).latent_dist.sample().detach()
                            ...
                            init_latents_w_fft = torch.fft.fftshift(torch.fft.fft2(noisy_latents.float()), dim=(-1, -2))
                            init_latents_w_fft[mask] = opt_wm[mask].clone()
                            init_latents_w_fft.requires_grad_()
                            init_latents_w_fft.retain_grad()
                            noisy_latents = torch.fft.ifft2(torch.fft.ifftshift(init_latents_w_fft, dim=(-1, -2))).real.float()
                        ...
                        with torch.autocast(device_type=accelerator.device.type, enabled=False):
                            x0_latents = scheduler.convert_model_output(noise_pred.float(), timesteps.item(), noisy_latents.float())  # predict x0 in one-step
    ...
    def decode_latents_wgrad(self, latents):
        latents = (1 / 0.18215 * latents).float()
