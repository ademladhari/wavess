...
def load_open_clip_model_preprocess_and_tokenizer(device: torch.device | None = None):
    if device is None:
...
def compute_clip_score(images, prompts, models, device: torch.device | None = None):
    if device is None:
        device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
